...
class WebsocketServer:
    ...
    def make_consumer(self, consumer=None):
        if consumer is None:
            self._consumer = {
                'func':
                WebsocketServer._default_consumer,
                'desc':
                "This consumer append 'Hello' before \
        ...
        self._server = websockets.serve(partial_consumer_handler, self._url,
                                        self._port)
    ...
    def make_producer(self, producer=None):
        ...
        self._server = websockets.serve(partial_producer_handler, self._url,
                                        self._port)
        self._description = self._producer['desc']
        return self

    def customize(self, description, **kwargs):
        partial_producer_handler = functools.partial(self._producer_handler,
                                                     **kwargs)
        self._server = websockets.serve(partial_producer_handler, self._url,
                                        self._port)
...
if __name__ == '__main__':
    ...
    loop = asyncio.new_event_loop()
    server.make_producer().init(loop=loop)
    loop.run_forever()
