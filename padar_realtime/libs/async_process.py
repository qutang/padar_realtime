...
Implementing async process of a function running on a different CPU core for
...
class AsyncMultiProcess:
    ...
    async def run(self, list_of_funcs, list_of_args):
        ...
        tasks = map(lambda pair: loop.run_in_executor(
            self._executor, pair[0], *(pair[1])),
            zip(list_of_funcs, list_of_args))
...
if __name__ == '__main__':
    ...
    def example1():
        print('Run multiple processes on different cores simultaneously \
              in a single call')
        print(str(os.getpid()))
        p = AsyncMultiProcess(n_cores=2)
        asyncio.get_event_loop().run_until_complete(p.run(
            [AsyncMultiProcess.default_function,
                AsyncMultiProcess.default_function],
            [[1, 3], [3, -1]]
        ))
    ...
    def example2():
        print('Run multiple processes on different cores sequentially \
         in multiple calls')
        print(str(os.getpid()))
        p = AsyncMultiProcess(n_cores=2)
        result1 = asyncio.get_event_loop().run_until_complete(
            p.run([AsyncMultiProcess.default_function], [[1, 3]]))
        result1.add_done_callback(lambda result: print(result1.result()))
        result2 = asyncio.get_event_loop().run_until_complete(
            p.run([AsyncMultiProcess.default_function], [[3, -1]]))
