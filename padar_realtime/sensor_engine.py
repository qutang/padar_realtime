...
class SensorEngine:
    ...
    def produce_accelerometer(self, grange, sr,
                              socket='websocket', activity='random'):
        if activity == 'random':
            def data_func(): return random() * grange * 2 - grange
        else:
            raise NotImplementedError(
                'Unrecognized activity argument' + activity)
        if socket == 'websocket':
            async def _producer():
                ...
                if self._last_ts == 0:
                    ...
                    logger.info('Real sampling rate: ' +
                                str(self._real_sr) + " Hz")
                    self._real_sr = 0
                    self._last_ts = ts
                return data
            self._server.make_producer({
                'func': _producer,
                'desc': 'Producer for ' + activity +
                ' accelerometer signal at ' +
                str(sr) + ' Hz and ' + str(grange) + ' G range.',
                'kwargs': {}
            }).start()
        else:
