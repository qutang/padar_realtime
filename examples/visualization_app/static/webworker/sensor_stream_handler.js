var SensorStreamHandler = function (url, port, rate) {
    this._url = url;
    this._port = port;
    this._rate = rate;
    this._data_buffer = [];
...
onmessage = function (e) {
    if (e.data['action'] == 'start') {
        ...
        var selected_handler = sensor_stream_handlers.filter(function (handler) {
            return handler._port == e.data['port']
        });
        if (selected_handler.length == 0) {
            console.log('Server not found, create a new one for: ' + port);
            selected_handler = new SensorStreamHandler(url, port, rate);
            sensor_stream_handlers.push(selected_handler);
        } else {
            selected_handler = selected_handler[0];
            console.log('Found a server for: ' + port);
        }
        console.log('Total data handlers: ' + sensor_stream_handlers.length);
